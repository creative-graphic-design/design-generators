...
The commands use `CUDA_VISIBLE_DEVICES=<gpu-id>`; replace `<gpu-id>` with
...
```bash
CUDA_VISIBLE_DEVICES=<gpu-id> uv run --package lace python models/lace/scripts/generate_reference.py \
  --dataset publaynet \
...
```bash
CUDA_VISIBLE_DEVICES=<gpu-id> uv run --package lace pytest \
...
```bash
CUDA_VISIBLE_DEVICES=<gpu-id> uv run --package lace python models/lace/scripts/smoke_from_pretrained.py \
