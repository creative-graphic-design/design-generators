...
The script sets `torch.use_deterministic_algorithms(True)`, disables TF32, and disables cuDNN. With cuDNN enabled, the first cross-process drift appears at the scorer's first ShuffleNetV2 convolution (`Feat_ext.feature3.0.0`) by a few float32 ULPs even with deterministic algorithms enabled.

```bash
CUDA_VISIBLE_DEVICES=2 uv run --package smarttext --extra vendor python models/smarttext/scripts/generate_reference_outputs.py \
...
```bash
CUDA_VISIBLE_DEVICES=2 uv run --package smarttext --extra vendor --with pytest --with pytest-cov pytest models/smarttext/tests/vendor_parity -m vendor_parity --no-cov
```
