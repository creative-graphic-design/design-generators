...
LayoutDM uses the original training recipe with the package-local denoiser initialization matched to the original `normal_(0, 0.02)` linear and embedding scheme. Scheduler cadence and optimizer-step behavior are covered by the S2/S3 checks when those checks are run.

## Seed Policy

RICO25 S5 evidence is reported at `training-seed n=8`. PubLayNet S5 evidence is limited to `training-seed n=3`; an n=8 expansion was attempted and stopped after the n=3 early verdict. Conditional RICO25 metrics reuse the same n=8 trained seed set without additional retraining.
...
LayoutDM package-local training currently has exact S0-S2 numeric parity on a fixed PubLayNet-style synthetic batch, plus S4 tokenizer/loader row encoding parity and preprocessed stream reader parity for local fixtures. RICO25 S5 training-seed n=8 and PubLayNet S5 training-seed n=3 have been evaluated under the original FIDNetV3 protocol. Matching the original `normal_(0, 0.02)` initialization for linear and embedding weights gives a RICO25 FID mean gap of +0.1252 at n=8; the FID ranges overlap, and Welch's t-test does not reject equality (`t=0.7331`, `p=0.4768`). With PyTorch-default initialization, the corresponding n=8 run had a larger gap (`ours=9.0264`, `vendor=7.1055`, `delta=+1.9209`, `p=0.0005`, no range overlap). The initialization match therefore resolves the established RICO25 over-FID failure.

PubLayNet's reported evidence is limited to training-seed n=3; an n=8 expansion was attempted and stopped after the n=3 early verdict. PubLayNet ours-initfix has lower FID than the original-code run (`delta=-0.5308`, Welch `t=-2.9589`, `p=0.0426`, no FID range overlap), so this result should not be described as statistically identical. The difference is in the favorable direction, and Alignment, Overlap, and mIoU do not show a significant gap (`p>0.18` with overlapping ranges). The overall S5 verdict is complete: RICO25 is statistically reproduced at n=8, and PubLayNet is accepted at n=3 with no over-FID regression or failure in which the package performs worse than the original implementation.
...
| --------- | -------------------------------------------------------------: | ----------------------------------------------------------------: |
| FID       |                                              6.7755 +/- 0.1298 |                                                13.3220 +/- 0.3105 |
| Alignment |                                              0.0024 +/- 0.0011 |                                                 0.0022 +/- 0.0000 |
| Overlap   |                                              0.8372 +/- 0.0161 |                                                 0.1386 +/- 0.0026 |
| mIoU      |                                              0.1896 +/- 0.0011 |                                                 0.0768 +/- 0.0016 |

| Metric      | RICO25 `c` authors' released-checkpoint reference (reference only) | RICO25 `partial` authors' released-checkpoint reference (reference only) | RICO25 `refinement` authors' released-checkpoint reference (reference only) |
| ----------- | -----------------------------------------------------------------: | -----------------------------------------------------------------------: | --------------------------------------------------------------------------: |
| FID         |                                                  3.4540 +/- 0.0931 |                                                        8.9231 +/- 0.2557 |                                                           4.8509 +/- 0.1255 |
| maximum_iou |                                                  0.2752 +/- 0.0006 |                                                        0.5872 +/- 0.0165 |                                                           0.3414 +/- 0.0007 |
| DocSim      |                                                  0.1674 +/- 0.0005 |                                                        0.0868 +/- 0.0028 |                                                           0.1971 +/- 0.0000 |
