...
`models/layout-dm/scripts/convert_original_checkpoint.py` writes a Hub-style `README.md` model card into each `save_pretrained` output. The card is generated by `laygen.common.model_card.layoutdm_model_card()` with `huggingface_hub.ModelCard.from_template()`, follows the Hugging Face annotated model card section structure, validates metadata with `ModelCard.validate()`, and includes usage, training data, evaluation parity results, citation, and the original implementation link.
