...
for task in elem pos attr img txt; do NVIDIA_TF32_OVERRIDE=0 CUDA_VISIBLE_DEVICES=<gpu-id> uv run --package flex-dm --extra vendor-tf python models/flex-dm/scripts/probe_layer_parity.py --mode export-vendor --dataset crello --task "$task" --probe-dir ".cache/flex-dm/probes/crello-$task" --disable-tf32; done
...
CUDA_VISIBLE_DEVICES=<gpu-id> uv run --package flex-dm pytest models/flex-dm/tests/vendor_parity -m vendor_parity
```
...
for task in elem pos attr; do NVIDIA_TF32_OVERRIDE=0 CUDA_VISIBLE_DEVICES=<gpu-id> uv run --package flex-dm --extra vendor-tf python models/flex-dm/scripts/probe_layer_parity.py --mode export-vendor --dataset rico --task "$task" --probe-dir ".cache/flex-dm/probes/rico-$task" --disable-tf32; done
uv pip uninstall nvidia-cublas-cu12 nvidia-cuda-cupti-cu12 nvidia-cuda-nvcc-cu12 nvidia-cuda-nvrtc-cu12 nvidia-cuda-runtime-cu12 nvidia-cudnn-cu12 nvidia-cufft-cu12 nvidia-curand-cu12 nvidia-cusolver-cu12 nvidia-cusparse-cu12 nvidia-nccl-cu12 nvidia-nvjitlink-cu12
uv pip install --reinstall nvidia-nccl-cu13==2.29.7
