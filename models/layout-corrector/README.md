...
This package ports [Layout-Corrector](https://arxiv.org/abs/2409.16689), a training-free confidence model that corrects layouts generated by [LayoutDM](models/layout-dm/README.md), the companion [`🧨diffusers`](https://huggingface.co/docs/diffusers/index)-style package for discrete diffusion layout generation.

## Model Details

### Model Description

Layout-Corrector refines candidate layouts by running a training-free correction stage on top of [LayoutDM](models/layout-dm/README.md), a discrete diffusion generator that samples layout tokens through a denoising loop. It accepts starter layouts or nested LayoutDM samples and returns corrected element boxes, labels, masks, and optional correction traces. Public outputs use normalized center `xywh` boxes in `[0, 1]`, dataset-local integer labels, a valid-element `mask`, and `id2label`.
