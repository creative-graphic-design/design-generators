...
| Magazine | not run | `not-yet-run (see artifact summary)` | not yet run | not yet run | not yet run | Pending upstream support for the Magazine dataset and polygon/train-only handling; see the [implementation checklist](https://github.com/creative-graphic-design/design-generators/blob/main/docs/implementation-checklist.md). |
