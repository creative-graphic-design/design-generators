...
The CGL label-conditioned path has completed the same staged validation ladder, the mandatory 300-step real-scale lockstep probe, three matched package/vendor training pairs, and the original evaluation protocol. The ledger-backed 15-metric comparison, seed ranges, and Welch tests are recorded in [CGL label S5 comparison](#cgl-label-s5-comparison); the final label verdict remains a maintainer decision and is intentionally left as a placeholder there. PKU and the other CGL conditioning modes are not claimed because this document contains no corresponding staged and full-run evidence.
...
| S5    | Parameterized `setsid nohup` launchers under `.cache/ralf/training-reproduction/cgl/label/` (stdin `/dev/null`, `OMP_NUM_THREADS=4`, seeds 1-3, 50 epochs, batch size 32)                                                                                                                                                                                                                                                                                                                                                                                      | `.cache/ralf/training-reproduction/cgl/label/s5/seed-1-run-002/launch-manifest.txt`; `.cache/ralf/training-reproduction/cgl/label/s5/seed-2-3-launch-manifest.txt`; `.cache/ralf/training-reproduction/cgl/label/s5/evaluation-ledger.txt` | `CHECK`; all three matched training pairs and all six original-protocol evaluations completed, with the formal comparison and final verdict below.                                                                                                               |
...
Seeds 2 and 3 completed with the launch metadata and first-30-minute health evidence recorded in `.cache/ralf/training-reproduction/cgl/label/s5/seed-2-3-launch-manifest.txt`; the package endpoint evidence and the vendor endpoint evidence are recorded in `.cache/ralf/training-reproduction/cgl/label/s5/evaluation-ledger.txt`.

### CGL label S5 comparison

Each system trained three matched seeds for 50 epochs with batch size `32`, training `num_workers=4`, and `OMP_NUM_THREADS=4`; each final checkpoint used the original evaluation chain with `cond_type=c`, `test_split=test`, `top_k=5`, temperature `1.0`, inference seeds `0/1/2`, evaluator `--run-on-local`, and inference/evaluation `num_workers=0`. The table reads the `average.test` values from the six ledger-verified `scores_all.yaml` files. Package mean in vendor range means the package mean is within the vendor seed minimum and maximum; vendor mean in package range is the reverse check. Welch p is the two-sided unequal-variance t-test with three package and three vendor training seeds.

| Metric                          | Package seeds 1 / 2 / 3                       | Vendor seeds 1 / 2 / 3                        | Package mean [min, max]                      | Vendor mean [min, max]                       | Package mean in vendor range | Vendor mean in package range |     Welch p |
| ------------------------------- | --------------------------------------------- | --------------------------------------------- | -------------------------------------------- | -------------------------------------------- | ---------------------------- | ---------------------------- | ----------: |
| `R_{shm} (vgg distance)`        | 14.3511895095 / 14.3878863891 / 14.3833064146 | 14.4073496276 / 14.2975033884 / 14.3876863624 | 14.3741274378 [14.3511895095, 14.3878863891] | 14.3641797928 [14.2975033884, 14.4073496276] | yes                          | yes                          |   0.8024298 |
| `alignment-LayoutGAN++`         | 0.0022843614 / 0.0021170891 / 0.0022345276    | 0.0021052188 / 0.0023873322 / 0.0022969461    | 0.0022119927 [0.0021170891, 0.0022843614]    | 0.0022631657 [0.0021052188, 0.0023873322]    | yes                          | yes                          |  0.63103109 |
| `occlusion`                     | 0.1253348302 / 0.1265282280 / 0.1264241272    | 0.1261307366 / 0.1257487252 / 0.1270315805    | 0.1260957284 [0.1253348302, 0.1265282280]    | 0.1263036808 [0.1257487252, 0.1270315805]    | yes                          | yes                          |  0.71916136 |
| `overlap-LayoutGAN++`           | 0.4015947368 / 0.3986948429 / 0.3998898768    | 0.4010803924 / 0.4030942369 / 0.3979678112    | 0.4000598188 [0.3986948429, 0.4015947368]    | 0.4007141468 [0.3979678112, 0.4030942369]    | yes                          | yes                          |  0.72663163 |
| `overlay`                       | 0.0059962839 / 0.0059919185 / 0.0061411700    | 0.0057005314 / 0.0063977108 / 0.0057240888    | 0.0060431241 [0.0059919185, 0.0061411700]    | 0.0059407770 [0.0057005314, 0.0063977108]    | yes                          | no                           |  0.70105925 |
| `test_coverage_layout`          | 0.9876152394 / 0.9817838498 / 0.9849494613    | 0.9852826835 / 0.9843940909 / 0.9869487948    | 0.9847828502 [0.9817838498, 0.9876152394]    | 0.9855418564 [0.9843940909, 0.9869487948]    | yes                          | yes                          |  0.71052094 |
| `test_density_layout`           | 1.1070865267 / 1.1179384650 / 1.1274908364    | 1.1053870932 / 1.1055870265 / 1.1229367988    | 1.1175052760 [1.1070865267, 1.1274908364]    | 1.1113036395 [1.1053870932, 1.1229367988]    | yes                          | yes                          |  0.49556803 |
| `test_fid_layout`               | 0.5411223665 / 0.6469444839 / 0.6043896710    | 0.5932227000 / 0.5050423065 / 0.5657610434    | 0.5974855071 [0.5411223665, 0.6469444839]    | 0.5546753500 [0.5050423065, 0.5932227000]    | no                           | yes                          |  0.34944199 |
| `test_precision_layout`         | 0.9736754415 / 0.9736199045 / 0.9770632012    | 0.9751749417 / 0.9735088304 / 0.9753970899    | 0.9747861824 [0.9736199045, 0.9770632012]    | 0.9746936206 [0.9735088304, 0.9753970899]    | yes                          | yes                          |  0.94708743 |
| `test_recall_layout`            | 0.9648450516 / 0.9590136621 / 0.9629012551    | 0.9653448850 / 0.9608463845 / 0.9617349772    | 0.9622533230 [0.9590136621, 0.9648450516]    | 0.9626420823 [0.9608463845, 0.9653448850]    | yes                          | yes                          |  0.86858255 |
| `underlay_effectiveness_loose`  | 0.9875862934 / 0.9882425311 / 0.9872631724    | 0.9866970034 / 0.9880854661 / 0.9874400695    | 0.9876973323 [0.9872631724, 0.9882425311]    | 0.9874075130 [0.9866970034, 0.9880854661]    | yes                          | yes                          |   0.5918706 |
| `underlay_effectiveness_strict` | 0.9668866396 / 0.9685819348 / 0.9663355748    | 0.9662394921 / 0.9689301252 / 0.9688400825    | 0.9672680497 [0.9663355748, 0.9685819348]    | 0.9680032333 [0.9662394921, 0.9689301252]    | yes                          | yes                          |  0.54676051 |
| `unreadability`                 | 0.0181477342 / 0.0181780296 / 0.0182463784    | 0.0181963420 / 0.0182018818 / 0.0183630453    | 0.0181907140 [0.0181477342, 0.0182463784]    | 0.0182537564 [0.0181963420, 0.0183630453]    | no                           | no                           |  0.38268397 |
| `utilization`                   | 0.1978437018 / 0.1968905906 / 0.1969332554    | 0.1996548403 / 0.1966746489 / 0.1967610033    | 0.1972225159 [0.1968905906, 0.1978437018]    | 0.1976968308 [0.1966746489, 0.1996548403]    | yes                          | yes                          |  0.68289267 |
| `validity`                      | 0.9983923759 / 0.9985774262 / 0.9985080323    | 0.9987971461 / 0.9988665672 / 0.9987740421    | 0.9984926115 [0.9983923759, 0.9985774262]    | 0.9988125851 [0.9987740421, 0.9988665672]    | no                           | no                           | 0.013410142 |

The six score artifacts and their ledger-recorded SHA-256 values are:
...
| CGL label         | package/vendor staged path | `blocked (maintainer verdict pending after formal comparison)`                                                                                                         | `training-seed n=3` | CGL label S0-S4 and 300-step lockstep passed; the 15-metric seed comparison and Welch p-values are recorded below                                              | Seed 1 vendor/package val loss `2.3233/2.303`; seed 2 `2.3271/2.357`; seed 3 `2.3442/2.319`        | `.cache/ralf/training-reproduction/cgl/label/s5/`                                       |
