...
- Set `CUDA_VISIBLE_DEVICES=<gpu-id>` to one CUDA device for the vendor reference/parity run.
...
```bash
CUDA_VISIBLE_DEVICES=<gpu-id> uv run --package coarse-to-fine --extra vendor python models/coarse-to-fine/scripts/export_reference.py \
  --dataset rico25 \
...
```bash
CUDA_VISIBLE_DEVICES=<gpu-id> uv run --package coarse-to-fine pytest \
