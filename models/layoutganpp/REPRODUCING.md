...
Step 2 generates golden vendor fixtures with the const-layout code. The generated files are `.cache/layoutganpp/fixtures/<dataset>/reference_seed0.pt`. Set `CUDA_VISIBLE_DEVICES=<gpu-id>` to the GPU you want to use.

```bash
CUDA_VISIBLE_DEVICES=<gpu-id> uv run --package layoutganpp python models/layoutganpp/scripts/generate_reference.py \
...
```bash
CUDA_VISIBLE_DEVICES=<gpu-id> uv run --package layoutganpp pytest models/layoutganpp/tests/vendor_parity -m vendor_parity -q
```
