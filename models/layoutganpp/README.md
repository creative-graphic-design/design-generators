...
Transformers-style LayoutGAN++ conversion package for `Constrained Graphic Layout Generation via Latent Optimization` (ACM MM 2021). It ports the released generator checkpoints from the original const-layout implementation and returns canonical `laygen.common.outputs.LayoutGenerationOutput` objects.
