...
- ✅ **Reference-verified**: our ports are numerically checked against the [original implementations](https://github.com/creative-graphic-design/design-generators/tree/main/vendor/) through [agreement checks](https://creative-graphic-design.github.io/design-generators/training-reproduction/) with [copy-pasteable reproduction commands per package](https://creative-graphic-design.github.io/design-generators/#models).
...
Every port is verified against the original implementation: agreement checks (the repository's `vendor_parity` tests) compare package outputs with references generated by running the original implementation in [`vendor/`](vendor/). Each package's `REPRODUCING.md` shows how to reproduce the check, and its README reports the resulting comparison numbers.
