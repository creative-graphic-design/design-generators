...
- [ ] Read the [umbrella plan](https://github.com/creative-graphic-design/design-generators/issues/2) body and its comments on unified interfaces v1 and v2, shared discrete diffusion, decisions, organization datasets, and review findings.
...
- [ ] Import `LayoutGenerationOutput` from `laygen.common.outputs` in the `laygen` package, preserve all eight fields (`bbox`, `labels`, `mask`, `id2label`, `sequences`, `scores`, `trajectory`, and `intermediates`), use the canonical `transformers.utils.ModelOutput`-based class for Transformers models, use the `outputs_diffusers` variant based on `diffusers.utils.BaseOutput` for Diffusers pipelines, build both from one field specification, keep no local copies, do not add an `extras` field, and put auxiliary data in `intermediates`.
...
- [ ] Expose the full agreed v1 pipeline `__call__` signature and the relevant v2 additions even when the model rejects some inputs.
...
- [ ] Expose only standard model entry points (`forward` and token-level `generate`) on model classes; put processor encoding, generation, decoding, layout-level orchestration, and the `LayoutGenerationOutput` result in the pipeline's `__call__`, and do not add `generate_layout`-style model methods.
...
- [ ] Respect the pinned dataset configurations: Rico uses `name="ui-screenshots-and-hierarchies-with-semantic-annotations"` with the default metadata-only configuration and a vendor-derived mapping for RICO13, PKU filters or reserves `INVALID` and uses pixel `ltrb` boxes, Magazine converts polygons to boxes and is train-only, and CGL-v2 uses `ralf-style` for validation and saliency.
...
- [ ] Run root pytest with `--import-mode=importlib` from the root `pyproject.toml` `addopts` setting, and preserve that setting when resolving pyproject merge conflicts because packages share test basenames.
...
- [ ] Ship a `README.md` for every library package under `lib/laygen`, `lib/posgen`, and future `lib/*` packages that explains its purpose, module map, key API examples, design rules, single-field-spec and no-`extras` constraints, extraction criteria, and links to the [umbrella plan](https://github.com/creative-graphic-design/design-generators/issues/2) and [shared library architecture](architecture.md).
...
- [ ] Give every model README a top-level `## Reproducibility` section that opens by stating how to reproduce agreement checks against the original implementation, then provides copy-pasteable commands for download, reference or golden generation with `CUDA_VISIBLE_DEVICES`, `pytest -m vendor_parity`, checkpoint conversion, and `from_pretrained` smoke tests, with prerequisites, cache locations, and expected artifacts.
...
- [ ] Apply the typing rules in the code and review safeguards below to closed string-mode sets, exhaustive branches, module constants, public signatures, and structured specification data.
...
- [ ] Do not commit machine-specific absolute paths such as `<repo-root>/...`; resolve script defaults relative to the repository root and provide an explicit CLI override, and use repository-relative paths such as `./vendor/<repo>` in documentation.
...
- [ ] Ship every model package README in model-card style with an overview, install and usage snippet using `from_pretrained` or a pipeline call, supported Hub ids, datasets, a numbered original-implementation agreement summary, and the original implementation's license and citation.
- [ ] Give every model README a top-level `## Parity Results` section with a numeric table stating what was compared, the number of cases, the match criterion, and the result.
