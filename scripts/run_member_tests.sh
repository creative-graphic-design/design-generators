...
#   Set `MEMBER_TEST_COVERAGE_JSON_DIR` to write pytest-cov JSON for CI
#   aggregation.
...
PY
  ...
  if [ -n "${MEMBER_TEST_COVERAGE_JSON_DIR:-}" ]; then
    mkdir -p "${MEMBER_TEST_COVERAGE_JSON_DIR}"
    pytest_args+=(
      "--cov-report=json:${MEMBER_TEST_COVERAGE_JSON_DIR}/${package_name}.json"
    )
  fi
fi
