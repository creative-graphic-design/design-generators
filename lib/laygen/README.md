...
`laygen` contains shared layout-generation schemas and utilities for the workspace. The core package is intentionally torch-free; tensor-backed helpers, agent integrations, and Diffusers adapters are available through extras declared in `lib/laygen/pyproject.toml`.
...
Use schema and normalization helpers without importing PyTorch or Diffusers.
...
- Transformers-side pipelines subclass `laygen.pipelines.LayoutGenerationPipeline`.
...
- Issue #2 tracks the umbrella model split and dataset policy.
- Issue #64 defines the shared library structure.
- Issue #81 defines the `laygen.nn` and `laygen.schedulers` shared-module restructuring.
