    ...
    *,
    ...
    metrics = parity_metrics or [
        ParityMetric(
            ...
            tokenizer_exact="pending",
            deterministic_exact="pending",
            logits_max_abs=0.0,
            logits_max_rel=0.0,
        )
    ]
...
"""
    return build_layout_model_card(
        ...
        intended_uses=(
            "Use this checkpoint for research and evaluation of token correction "
            "during discrete diffusion layout generation."
        ),
        limitations=(
            "The converted checkpoint follows the original Layout-Corrector release "
            "and is intended for layout synthesis research, not image rendering."
        ),
        how_to_use=how_to_use,
        training_data=(
            f"The original checkpoint was trained on `{dataset_id}` as released by "
            "the original Layout-Corrector project."
        ),
        parity_metrics=metrics,
...
@inproceedings{kamata2024layoutcorrector,
  title = {Layout-Corrector: Alleviating Layout Sticking Phenomenon in Discrete Diffusion Model},
  author = {Kamata, Hiromu and Kato, Yuki and Kikuchi, Kotaro and Nakazawa, Akihiro},
  booktitle = {ECCV},
  year = {2024}
}
"""
