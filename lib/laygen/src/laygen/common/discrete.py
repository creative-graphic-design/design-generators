...
def top_k_logits(logits: Float[Tensor, "... vocab"], k: int, dim: int = -1) -> Tensor:
...
def _top_p_logits(logits: Float[Tensor, "... vocab"], top_p: float) -> Tensor:
    import torch
...
def batch_topk_mask(
    scores: Float[Tensor, "batch candidates"], k: Int[Tensor, "batch"]
) -> Tensor:
